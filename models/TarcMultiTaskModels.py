...
from fairseq.models.TarcTransformerModules import TarcTransformerDecoder
...
class TarcMultiTaskModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.sub_task == 'tarc-full' and args.load_madar_data != 'None' and args.load_madar_model != 'None':
            ...
            # WARNING: THE FOLLOWING LINES OF CODE ARE TASK DEPENDENT, THEY WILL NOT WORK WHEN CHANGING DATA AND TASKS
            #           THIS WAS THE BEST INITIALIZATION FOUND WITH PRE-TRAINED MADAR MODEL.
            #           ONCE MADAR WILL BE ANNOTATED WITH ARABISH THIS WILL NOT BE NECESSARY ANY MORE.

            # Decoder 0: classification; Attentions: 1. encoder state
            decoders[0].layers.load_state_dict( madar_model.decoder.decoders[0].layers.state_dict() )
            # NOTE: decoders[0] has only one attention to the encoder's hidden state. This is different from the madar_model.decoders[0] hidden state, 
            #           as they don't process the same language. It should be thus preferable to not pre-initialize attentions here.
            #           However pre-initialization gave slightly better results.
            decoders[0].attentions.load_state_dict( madar_model.decoder.decoders[0].attentions.state_dict() )

            # WARNING: THE FOLLOWING LINES OF CODE FOR INITIALIZING decoders[1] PARAMETERS ARE EVEN PYTORCH VERSION (1.4.0) DEPENDENT, THINGS CHANGED FROM PYTORCH 1.5.0
            # ----------------------------------------------------------------------------------
            ih_pre_init_l0 = torch.cat( [madar_model.encoder.encoders[0].lstm.weight_ih_l0, madar_model.encoder.encoders[0].lstm.weight_ih_l0], -1 )
            ih_pre_init_l1 = madar_model.encoder.encoders[0].lstm.weight_hh_l1
            ih_pre_init_l2 = madar_model.encoder.encoders[0].lstm.weight_hh_l2 

            decoders[1].layers[0].weight_ih.data = ih_pre_init_l0
            decoders[1].layers[1].weight_ih.data = ih_pre_init_l1
            decoders[1].layers[2].weight_ih.data = ih_pre_init_l2

            decoders[1].layers[0].bias_ih.data = madar_model.encoder.encoders[0].lstm.bias_ih_l0.data
            decoders[1].layers[1].bias_ih.data = madar_model.encoder.encoders[0].lstm.bias_ih_l1.data
            decoders[1].layers[2].bias_ih.data = madar_model.encoder.encoders[0].lstm.bias_ih_l2.data

            decoders[1].layers[0].weight_hh.data = madar_model.encoder.encoders[0].lstm.weight_hh_l0.data
            decoders[1].layers[1].weight_hh.data = madar_model.encoder.encoders[0].lstm.weight_hh_l1.data
            decoders[1].layers[2].weight_hh.data = madar_model.encoder.encoders[0].lstm.weight_hh_l2.data

            decoders[1].layers[0].bias_hh.data = madar_model.encoder.encoders[0].lstm.bias_hh_l0.data
            decoders[1].layers[1].bias_hh.data = madar_model.encoder.encoders[0].lstm.bias_hh_l1.data
            decoders[1].layers[2].bias_hh.data = madar_model.encoder.encoders[0].lstm.bias_hh_l2.data 
            decoders[1].attentions[1].load_state_dict( madar_model.decoder.decoders[1].attentions[1].state_dict() )
            # -----------------------------------------------------------------------------------

            # Decoder 2: tokenization; Attentions: 1. encoder state, 2. classification, 3. transcription
            #     Loaded decoder 1: tokenization; attentions: 1. encoder state (transcription), 2. classification
            decoders[2].layers.load_state_dict( madar_model.decoder.decoders[1].layers.state_dict() )
            decoders[2].attentions[0].load_state_dict( madar_model.decoder.decoders[1].attentions[0].state_dict() )
            decoders[2].attentions[1].load_state_dict( madar_model.decoder.decoders[1].attentions[1].state_dict() )
            # See NOTE below. Moreover here we use the tokenisation attention of the second layer.
            decoders[2].attentions[2].load_state_dict( madar_model.decoder.decoders[2].attentions[2].state_dict() )
