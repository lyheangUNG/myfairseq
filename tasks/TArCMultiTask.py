...
def create_character_list(token, args, pad_flag=True, rep_flag=False):
    if (not (token in fillers + latin_fillers)) and rep_flag and args.sub_task in ['tarc-ext', 'tarc-full']:
        token = replace_all_num(token)
        token = replace_all_pun(token)
        token = replace_all_sym(token)
    elif (not (token in fillers + latin_fillers)) and args.sub_task in ['tarc-ext', 'tarc-full']:  
    ...
    if args.sub_task in ['tarc-ext', 'tarc-full']:
...
def check_column_processing(num_columns, args):
    ...
    if num_columns == 3 and not args.sub_task in ['tiger-mt', 'tiger-mt-ext', 'tarc-base', 'base', 'madar-trs']:
        ...
        raise ValueError(' wrong num. of columns in input data for processing mode {}'.format(args.sub_task))
    elif num_columns == 5 and not args.sub_task in ['tarc-full','madar-trs-full']:
        raise ValueError(' wrong num. of columns in input data for processing mode {}'.format(args.sub_task))
    elif (not num_columns in [3, 4, 5]) and not (args.sub_task in ['base', 'tarc-full']):
        raise ValueError(' Unexpected number of columns in input file, possible values: 3, 4, unless base is specified. Got {}'.format(num_columns))
...
def apply_global_filling(args, curr_seq, cls_seq, Afillers, Lfillers, OFFcols, idx):
    ...
    if args.sub_task in ['tarc-ext', 'tarc-full', 'madar-trs', 'madar-trs-ex', 'madar-trs-full'] and idx != CLS_COL_IDX and idx != POS_COL_IDX:
        for s_idx in range(len(curr_seq)):
            if cls_seq[s_idx] == 'foreign':
                if (args.sub_task == 'tarc-full' and idx == 0) or (args.sub_task == 'madar-trs' and idx == 2) or (args.sub_task == 'madar-trs-ex' and idx == 3) or (args.sub_task == 'madar-trs-full' and idx == 4):
        ...
        for s_idx in range(curr_seq_len):
            ...
            for idx in range(num_columns):
                if idx > 0 and args.ignore_test_output and test_flag:
                    ...
                    rep_flag = True
                    if (idx == 0 and args.sub_task == 'tarc-full') or (idx == 2 and args.sub_task == 'madar-trs') or (args.sub_task == 'madar-trs-ex' and idx == 3) or (args.sub_task == 'madar-trs-full' and idx == 4):
                        rep_flag = False
...
def read_tarc_parallel_data( file_prefix, args ):
    ...
    for i in range( len(input_data) ):
        ...
        for t in input_data[i]:
            rep_flag = True
            if (c_idx == 0 and args.sub_task == 'tarc-full') or (c_idx == 2 and args.sub_task == 'madar-trs') or (args.sub_task == 'madar-trs-ex' and c_idx == 3) or (args.sub_task == 'madar-trs-full' and c_idx == 4):
                rep_flag = False
            #process_res = column_processing[idx]( tokens[idx], args, rep_flag=rep_flag )
